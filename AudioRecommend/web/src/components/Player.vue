<template>
  <div class="moggy-player" @mousedown="closeList">
    ...
    <ul class="lyric" ref="playerLyric">
      ...
      <li></li>
      <li v-for="(l, index) in lyric" :key="index" :class="[index+1===upTime?'active':'']">{{l.lyr}}</li>
      <li></li>
      <li></li>
    </ul>
    ...
    <div ref="list" class="list" :class="isShowList?'show':'none'">
      ...
      <ul>
        ...
        <li
          ...
          :key="s.id"
          :class="songNum==index?'active':''"
          @click="changeSong(index,$event)"
        >
...
import { s_2_hs } from "../utils/tools";
export default {
  class: "moggy-player",
  data() {
    return {
  ...
  computed: {
    ...
    currentTime() {
      if (
        this.lyric.length != 0 &&
        this.lyric[this.upTime] &&
        Object.prototype.hasOwnProperty.call(this.lyric[this.upTime], "time")
      ) {
        ...
        //     let hight = this.$refs.playerLyricli.clientHeight;
        //     this.$refs.playerLyric.scrollTop =
        //       (this.$store.state.upTime - 1) * hight;
        //   }
        }
      }
    ...
    progress() {
      return {
        width: `${(this.$store.state.currentTime / this.$store.state.duration) *
          100}%`
      };
    }
  },
